...
public class ReadEmployees implements Operation {
	...
	public void oper(Database database, Scanner scanner) {
		...
		ArrayList<Employee> employees=getEmployees(database);
		for(Employee e: employees) {
			e.print();
		}
	}
	public ArrayList<Employee> getEmployees(Database database){
		...
		try {
			...
			ResultSet rs;
			rs = database.getStatement().executeQuery(select);
			while(rs.next()) {
				...
				deptIDs.add(rs.getInt("DepartmentID"));
			}
